package com.dull.bird.email.tool.execl;
...
public class ExeclAnalyze {
    ...
    private ExeclAnalyze(){}


    /**
     * 解析上传的execl
     * @param fileStream 传入的MultipartFile文件
     * @return  返回结果对象
     * @throws IOException  出现异常报错
     */
    public ExeclAnalyzeResult analyzeExecl(MultipartFile fileStream) throws IOException {
        ...
        return new ExeclAnalyzeResult(titles, bodies);
    }
     ...
     */
    public static ExeclAnalyze getInstance(){
    ...
    private static class ExeclAnalyzeInnerObject{
        private static final ExeclAnalyze execlAnalyze = new ExeclAnalyze();
    }
